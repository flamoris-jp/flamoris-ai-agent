...
sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "apps" / "umeko_chat"))

import chat
import db
from intelligence import IntelligenceClient, IntelligenceError
from register_runtime import register
...
class IntelligenceClientTests(unittest.TestCase):
    def test_resolves_served_id_and_builds_chat_payload(self):
        ...
        with patch("intelligence.requests.request", side_effect=[models, answer]) as request:
    ...
    def test_rejects_wrong_model_and_ambiguous_list(self):
        ...
        with patch("intelligence.requests.request", return_value=response):
    ...
    def test_network_and_invalid_response_fail_cleanly(self):
        ...
        with patch("intelligence.requests.request", side_effect=requests.ConnectionError("offline")):
...
class ModelProvenanceTests(unittest.TestCase):
    ...
    def test_registration_does_not_repoint_existing_model(self):
        ...
        with patch("register_runtime.platform.node", return_value="lime"):
            with self.assertRaisesRegex(RuntimeError, "different model"):
                register(conn, host_key="lime", model_key="llama.cpp:gpt-oss-20b", served_model="actual-id")
...
class UmekoSessionTests(unittest.TestCase):
    def test_failed_conversation_start_closes_instance(self):
        ...
        with (patch.object(chat, "IntelligenceClient", return_value=client),
              patch.object(chat, "get_connection", return_value=Mock()),
              patch.object(chat, "load_runtime_refs", return_value={
                  "agent_id": "agent", "project_id": "project", "model_id": "model-row"}),
    ...
    def test_message_persistence_previous_context_and_clean_exit(self):
        ...
        runtime = {
            "conversation_id": "new-conversation", "conversation_session_id": "session",
            "human_participant_id": "human", "agent_participant_id": "agent",
        }
        previous = {"conversation_id": "old-conversation", "messages": [
        ...
        self.assertEqual(save.call_args_list[1].kwargs["metadata"], {"intelligence_model": "actual-served-id"})
        self.assertEqual(client.chat.call_args.args[1][1]["content"], "こんにちは")
        close.assert_called_once_with(unittest.mock.ANY, conversation_id="new-conversation",
                                      conversation_session_id="session", instance_id="instance")

    def test_failed_completion_leaves_only_user_message_and_closes(self):
        refs = {"agent_id": "same-agent", "project_id": "project", "model_id": "model-row"}
        runtime = {"conversation_id": "conversation", "conversation_session_id": "session",
                   "human_participant_id": "human", "agent_participant_id": "agent"}
        client = Mock()
        client.resolve_model.return_value = "model-id"
